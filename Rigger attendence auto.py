...
try:
    ...
    columns_to_load = [
        ...
        "rigger_unique_id", "attendance_date", "attendance_status", 
        "timestamp_of_attendance_marked", "attendance_approval_status", 
        "attendance_approver_name", "attendance_approver_msisdn", 
        "timestamp_of_approval_action", "attendance_marked_latitude", 
        "attendance_marked_longitude", "full_body_safety_harness_availability", 
        "full_body_safety_harness_condition", "work_positioning_lanyard_availability", 
        "work_positioning_lanyard_condition", "safety_shoes_availability", 
        "safety_shoes_condition", "safety_helmet_availability", 
        "safety_helmet_condition", "cotton_gloves_availability", 
        "cotton_gloves_condition", "tool_kit_availability", "tool_kit_condition"
    ]
...
df_rigger_attendance = combined_data.drop(
    columns=[
        "attendance_approval_status", "attendance_approver_name", 
        "attendance_approver_msisdn", "timestamp_of_approval_action"
    ],
    errors="ignore"
)
...
df_attendance_summary.columns = [col.strftime('%d %b') if isinstance(col, (pd.Timestamp, datetime)) else col for col in df_attendance_summary.columns]
...
vendors = {
    ...
    "HTIS Pvt Ltd": "HTIS Pvt Ltd"  # Added as requested
}
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt == retries:
                ...
                logging.warning(f"⚠️ Email send failed, retrying ({attempt}/{retries})...")

# ============================ SEND VENDOR EMAILS ============================
email_body_vendor = """
<html><body><p>Dear Team,</p><p>Please find attached <strong>Rigger_Attendance Report</strong> for <strong>{vendor}</strong>.</p><p>Regards,<br>Central Team FSO</p></body></html>
"""
...
# ============================ SEND CENTRAL EMAIL ============================
email_body_central = """
<html><body><p>Dear Team,</p><p>Please find attached <strong>Rigger Attendance Report</strong> for the last 5 days.</p><p>Regards,<br>Central Team FSO</p></body></html>
"""
